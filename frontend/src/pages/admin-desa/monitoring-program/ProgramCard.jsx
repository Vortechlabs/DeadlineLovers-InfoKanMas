...
import { Eye, MapPin, User, Calendar, AlertCircle, FileText, Video, Image, Clock } from 'lucide-react';
...
const ProgramCard = ({ program, onViewDetails }) => {
  ...
  const daysRemaining = getDaysRemaining(program.tanggalSelesai);

  return (
    <div className="bg-white rounded-2xl border border-gray-200 hover:shadow-lg transition-all duration-300 overflow-hidden group cursor-pointer">
      {/* Header */}
          ...
          <div className="flex items-center gap-2">
            {program.issues > 0 && (
              <div className="flex items-center gap-1 text-red-600 text-xs">
                <AlertCircle size={12} />
                <span>{program.issues}</span>
              </div>
            )}
            {daysRemaining !== null && daysRemaining < 30 && program.status === 'dalam_pengerjaan' && (
              <div className="flex items-center gap-1 text-orange-600 text-xs">
                <Clock size={12} />
                <span>{daysRemaining}h</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="space-y-2 text-xs text-gray-600">
          ...
          <div className="flex items-center gap-2">
            <User size={12} />
            <span className="truncate">{program.penanggungJawab}</span>
          </div>
          <div className="flex items-center gap-2">
            <Calendar size={12} />
        ...
        <div className="bg-gray-50 rounded-lg p-2">
          <div className="flex justify-between items-center text-xs">
            ...
            <span className={`font-medium ${getPriorityColor(program.prioritas)}`}>
              {program.prioritas}
            </span>
          </div>
